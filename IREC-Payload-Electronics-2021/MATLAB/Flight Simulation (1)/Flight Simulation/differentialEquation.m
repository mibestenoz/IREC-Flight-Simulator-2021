...
%
%Outputs:
%xdot = [downrange velocity (m/s), downrange acceleration (m/s^2) , vertical velocity (m/s), vertical acceleration (m/s^2), mass burn rate (kg/s)]
...
xdot = real([x(2);(thrust*sin(launch_angle))/m;x(4);(thrust*cos(launch_angle)-m*g-.5*rho*x(4)^2*C_D*A)/m;-x(5)*thrust/I;.5*rho*((sqrt((x(2).^2)+(x(4).^2)))^2)*C_D*A]);
